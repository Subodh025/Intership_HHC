...
persons = []
tracker_logs = []
DATA_FILE = Path("tracker_data1.json")
...
def create_dress_from_input():
    color = input("Enter Dress Color: ")
    dress_type = input("Enter Dress Type: ")
    return Dress(color, dress_type)


def build_person_from_data(person_data):
    dress_data = person_data.get("dress")
    dress = None

    if dress_data:
        dress = Dress(
            dress_data.get("color", ""),
            dress_data.get("dress_type", ""),
        )

    return Person(
        person_data.get("face_embedding", ""),
        person_data.get("body_embedding", ""),
        dress,
    )
...
def load_from_json():
    ...
    for person_data in data.get("persons", []):
        persons.append(build_person_from_data(person_data))

    for log_data in data.get("tracker_logs", []):
        face_embedding = log_data.get("face_embedding", "")
        person = find_person(face_embedding)

        if person is None:
            person = build_person_from_data(log_data)

        tracker_logs.append(
            TrackerLog(
                person,
                log_data.get("entry_time", ""),
                log_data.get("exit_time", ""),
            )
        )
...
def save_to_json():
    ...
    for person in persons:
        data["persons"].append(person.serialize())

    for log in tracker_logs:
        data["tracker_logs"].append(log.serialize())
...
def add_dress():
    ...
    person.dress = create_dress_from_input()
